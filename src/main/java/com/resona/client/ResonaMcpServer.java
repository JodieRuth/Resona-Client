...
public class ResonaMcpServer {
        ...
        if (server != null) {
            for (Object obj : server.getConfigurationManager().playerEntityList) {
                ...
                item.addProperty(
                    ...
                    p.getUniqueID()
                        .toString());
                item.addProperty("name", p.getCommandSenderName());
    ...
    private JsonObject handleNearbyTileEntities(JsonObject request) {
        ...
        if (target.world != null) {
            ...
            for (Object obj : target.world.loadedTileEntityList) {
                TileEntity te = (TileEntity) obj;
                double dx = te.xCoord - target.x;
                double dy = te.yCoord - target.y;
                double dz = te.zCoord - target.z;
    ...
    private JsonObject handleRayTrace(JsonObject request) {
        ...
        MovingObjectPosition hit = player.rayTrace(distance, 1.0f);
        if (hit == null) {
        ...
        if (hit.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK) {
            ...
            JsonObject blockInfo = blockInfo((WorldServer) player.worldObj, hit.blockX, hit.blockY, hit.blockZ, block);
    ...
    private JsonObject handleBlockInfo(JsonObject request) {
        ...
        if (world != null) {
            ...
            JsonObject info = blockInfo(world, x, y, z, block);
            data.add("block", info);
        } else {
    ...
    private JsonObject handleSearchItems(JsonObject request) {
        ...
        JsonArray arr = new JsonArray();
        if (Item.itemRegistry.getKeys() != null) {
            for (Object keyObj : Item.itemRegistry.getKeys()) {
                ...
                try {
                    unloc = item.getUnlocalizedName();
                } catch (Exception e) {
                    unloc = "";
                }
                if (containsIgnoreCase(key, q) || containsIgnoreCase(name, q) || containsIgnoreCase(unloc, q)) {
    ...
    private JsonObject handleLookupItem(JsonObject request) {
        ...
        for (int i = 0; i < itemsToLookup.size(); i++) {
            ...
            if (identifier != null && !identifier.trim()
                ...
                if ("1.7.10".equals(mcVersion)) {
                    try {
                        ...
                        item = (Item) Item.itemRegistry.getObject(trimmed);
                    }
                    ...
                    if (lastColon > firstColon && lastColon > -1) {
                        ...
                        try {
                            ...
                            item = (Item) Item.itemRegistry.getObject(trimmed.substring(0, lastColon));
                        } catch (NumberFormatException e) {
                            item = (Item) Item.itemRegistry.getObject(trimmed);
                        }
                    } else {
            ...
            if (item != null) {
                ...
                JsonObject obj = new JsonObject();
                obj.addProperty("id", Item.getIdFromItem(item));
                obj.addProperty("key", getItemKey(item));
                obj.addProperty("unlocalized", item.getUnlocalizedName());
    ...
    private TargetPos resolveTarget(JsonObject request) {
        EntityPlayerMP player = findPlayer(getString(request, "player"));
        if (player != null) {
            return new TargetPos((WorldServer) player.worldObj, player.posX, player.posY, player.posZ);
        }
        int x = getInt(request, "x", 0);
        int y = getInt(request, "y", 0);
        int z = getInt(request, "z", 0);
    ...
    private JsonObject tileEntityInfo(WorldServer world, TileEntity te) {
        Block block = world.getBlock(te.xCoord, te.yCoord, te.zCoord);
        JsonObject obj = blockInfo(world, te.xCoord, te.yCoord, te.zCoord, block);
    ...
    private JsonObject blockInfo(WorldServer world, int x, int y, int z, Block block) {
        JsonObject obj = new JsonObject();
        if (block == null) {
        ...
        obj.addProperty("y", y);
        obj.addProperty("z", z);
        obj.addProperty("key", block.getUnlocalizedName());
    ...
    private JsonObject entityInfo(Entity entity) {
        ...
        obj.addProperty("name", entity.getCommandSenderName());
        obj.addProperty(
            "uuid",
            entity.getUniqueID()
                .toString());
    ...
    private JsonObject itemInfo(ItemStack stack, int slot) {
        ...
        obj.addProperty("key", key);
        obj.addProperty("name", stack.getDisplayName());
        obj.addProperty("count", stack.stackSize);
    ...
    private String getItemModId(Item item) {
        ...
        return id == null ? "minecraft" : id.modId;
    }
